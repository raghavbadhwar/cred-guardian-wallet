-- Add title column based on existing type column
ALTER TABLE credentials ADD COLUMN IF NOT EXISTS title TEXT;

-- Update title with type values for existing records
UPDATE credentials SET title = type WHERE title IS NULL;

-- Rename columns to match new schema (handle existing structure)
